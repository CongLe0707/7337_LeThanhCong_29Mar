...
let { SignUpValidator, validate } = require('../utils/validator'); // Import validators
...
router.post('/', SignUpValidator, validate, async function (req, res, next) { // Add validation middleware
  try {
    let body = req.body;
...
router.put('/:id', SignUpValidator, validate, async function (req, res, next) { // Add validation middleware
  try {
    let body = req.body;
